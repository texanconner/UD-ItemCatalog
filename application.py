...
# Queries the database for all of the categories and off the items
# in that category.
...
def showCategory(category_name):
    ...
    currentUser = getUserName()
    return render_template(
        'categoryItems.html', categories=categories,
        category=category, items=items, currentUser=currentUser)
...
def showItem(category_name, item_name):
    ...
    currentUser = getUserName()
    if (
        'email' not in login_session or item.created_by
        != login_session['email']
            ):
        return render_template(
        ...
        return render_template(
            'itemCreator.html', category=category,
...
# The created_by field is taken from the login session, and
# the category from the current category.
# By assigning the created_by field to the item, we forego the need to create
# a user data table.


@app.route('/catalog/category/<category_name>/items/new',
           methods=['GET', 'POST'])
def addItem(category_name):
    ...
    if request.method == 'POST':
        ...
        return redirect(url_for(
            'showCategory', category_name=category.name,
            currentUser=currentUser))
    else:
        return render_template(
            'addItem.html', category_name=category.name,
            category=category, currentUser=currentUser)
    return "Created New Item"


# Displays the edit item page.
# Queries the database for the current category and item.
# Checks if current user is the creator of the item(preventing
# direct URL access as well). If
...
@app.route('/catalog/category/<category_name>/items/<item_name>/edit/',
           methods=['GET', 'POST'])
def editItem(category_name, item_name):
    ...
    if request.method == 'POST':
        ...
        return render_template(
            'editItem.html', category_id=category.id, item_name=item.name,
            category=category, item=item, currentUser=currentUser)
...
@app.route('/catalog/category/<category_name>/items/<item_name>/delete/',
           methods=['GET', 'POST'])
def deleteItem(category_name, item_name):
    ...
    if request.method == 'POST':
        ...
        return render_template(
            'deleteItem.html', category_name=category.name,
            item_name=item.name, category=category, item=item,
            currentUser=currentUser)


# Displays the login page.
# The only current option is a google Oauth.
